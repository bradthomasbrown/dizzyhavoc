...
export class Burn {
  ...
  async state(): Promise<Error | BurnState> {
    ...
    if (result.reduce((p, c) => p + (c.versionstamp !== null ? 1 : 0), 0) > 1) {
      return new Error(`Burn.state: multiple mint states detected`);
    }
  ...
  async move(state: BurnState): Promise<Error | boolean> {
    ...
    if (state === "finalized") {
      ...
      if (state !== null) {
        return new Error(
          `Burn.move(\'finalized\'): cannot create mint with existing state ${state}`,
        );
      }
      const kvMint = mint.toJSON();
      atom.set(
        ["sendable", "mint", mint.chain.chainId, mint.burn.hash],
        kvMint,
      );
  ...
  receipt(): Promise<Error | Receipt> {
    return this.chain.receipt(this.hash);
  }
