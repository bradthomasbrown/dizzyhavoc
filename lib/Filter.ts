...
export class Filter {
    ...
    constructor(entry:Deno.KvEntryMaybe<KvFilter>, chain:Chain, address:string, topics:(string|string[])[]) {
    ...
    bisect() { this.toBlock = this.fromBlock + (this.toBlock - this.fromBlock) / 2n }

    bump() { this.fromBlock = this.toBlock + 1n }

    async update(processId:string):Promise<Error|boolean> {
        const kvFilter:KvFilter = { fromBlock: this.fromBlock, toBlock: this.toBlock }
        const result = await this.chain.kvv.atomic()
            .check(this.entry)
            .set(['filters', this.chain.chainId, this.address, this.topics.sort((a, b) => a < b ? -1 : a == b ? 0 : 1).join(), processId], kvFilter)
            .commit()
