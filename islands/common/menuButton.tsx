...
export default function MenuButton() {
  ...
  const isMobile = globalThis.matchMedia("(max-width: 768px)").matches;
    ...
    const handleCategoryChange = (category: string) => {
        ...
        {if(isMobile){return (
            <><div>
            <select class="text-2xl text-[#3d3d3d] dark:text-[#ccb286] text-center w-[40px] sm:w-[150px] shadow-lg font-[Poppins] rounded-lg hover:scale-[102%] border border-[#e9e9e9] dark:border-[#ffffff1f] cursor-pointer dark:bg-[#191919] bg-[#f1f1f1]" name="category" value={category} onChange={event => handleCategoryChange(event.target.value)}>
