...
export default async function({
    ...
    const code = (getCode('ERC20/ERC20_20240127_0.sol') as string)
    ...
    if (prompt(`erc20_0 gasLimit is ${gasLimit}`) != 'y') throw new Error(`erc20_0 gasLimit of ${gasLimit} not approved`)
    ...
    console.log('erc20_0', { address, hash })
    const erc20_0 = { address, hash, ...e.sendRawTx({ signedTx }) }
    if (execute) {
        e.ejrc({ url, ...erc20_0 })
        while (!await e.receipt({ ...erc20_0 }).call(url)) await new Promise(r => setTimeout(r, interval))
    }
    console.log('erc20_0', (await e.code({ ...erc20_0 }).call(url)).length)
    if (traceTx) {
        const temp = Deno.makeTempFileSync()
        Deno.writeTextFileSync(temp, JSON.stringify(await e.traceTx({ ...erc20_0 }).call(url), undefined, 4))
        console.log(temp)
    }
    if (traceCall) {
        const temp = Deno.makeTempFileSync()
